...
describe('createVocabChunks', () => {
  ...
  it('should chunk language modules that are only statically imported by a preload module', () => {
    const modules: Record<
      string,
      { isEntry: boolean; dynamicImporters: string[]; importers: string[] }
    > = {
      'virtual:vocab-en-1a2b3c4d.js?source=abc': {
        isEntry: false,
        dynamicImporters: [],
        importers: ['virtual:vocab-preload-en'],
      },
      'virtual:vocab-preload-en': {
        isEntry: false,
        dynamicImporters: ['src/App.vocab/index.ts'],
        importers: [],
      },
      'src/App.vocab/index.ts': {
        isEntry: false,
        dynamicImporters: [],
        importers: ['app.js'],
      },
    ...
    const ctx = {
      getModuleInfo: vi.fn((id: string) => modules[id]),
    } as unknown as ChunkingContext;

    expect(
      createVocabChunks('virtual:vocab-en-1a2b3c4d.js?source=abc', ctx),
    ).toBe('en-translations');
    expect(createVocabChunks('virtual:vocab-preload-en', ctx)).toBe(
      'en-translations',
    );
  });
});
